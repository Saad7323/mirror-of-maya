...
def find_optimal_threshold(similarity_scores, true_labels, step=0.01): #Finds the optimal threshold that maximizes F1 score on the given data.
    #how much to increment the threshold each time and defualt = 0.01
    similarity_scores = np.array(similarity_scores) #this is an array of similarity scores (float values bw 0.0 - 1.0)
    true_labels = np.array(true_labels) #array of true labels, where 1 = duplicate, 0 = non duplicate
